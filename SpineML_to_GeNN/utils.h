...
// Traub and Miles HH neurons
 ...
 //Izhikevich neurons
  ...
  n.varNames.push_back(tS("V"));
  n.varTypes.push_back(tS("float"));
  n.varNames.push_back(tS("tSpike"));
  n.varTypes.push_back(tS("float"));
  n.varNames.push_back(tS("__regime_val"));
  n.varTypes.push_back(tS("int"));
  n.pNames.clear();
  
  n.pNames.push_back(tS("VReset"));
  n.pNames.push_back(tS("VThresh"));
  n.pNames.push_back(tS("tRefrac"));
  n.pNames.push_back(tS("VRest"));
  n.pNames.push_back(tS("TAUm"));
  n.pNames.push_back(tS("Cm"));
  n.dpNames.clear();

  n.simCode = tS(" \
  	 if ($(__regime_val)==1) { \n \
$(V) += ($(Isyn)/$(Cm)+($(VRest)-$(V))/$(TAUm))*DT; \n \
	 	if ($(V)>$(VThresh)) { \n \
$(V) = $(VReset); \n \
$(tSpike) = t; \n \
$(__regime_val) = 2; \n \

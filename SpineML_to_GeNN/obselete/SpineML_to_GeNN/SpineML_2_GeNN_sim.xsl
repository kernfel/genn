...
// INIT CODE
 ...
 void *devPtr;
 FILE * output_file;
 output_file = fopen("/Users/alex/genn/log1.dat","w");
 if (!output_file)
    ...
    if (which == GPU) {
       ...
       <!----> t);       	
		if (!(i % 1)) {
			...
			float out3;
      		CHECK_CUDA_ERRORS((cudaMemcpy(&amp;out3, d_moo_PSSynapse2_PopulationS1_to_PopulationS2, 1*sizeof(float), cudaMemcpyDeviceToHost)));
